...
END:VCARD
    ...
    conn.reply = (jid, text = '', quoted, options) => {
        ...
        return global.db.data.settings.thumb ? conn.sendMessage(jid, { text, mentions: conn.parseMention(text), contextInfo: {
    externalAdReply :{
   ...
   }} }, { quoted, ...options }) : conn.sendMessage(jid, { ...options, text }, { quoted, ...options })
 }
...
return await conn.sendMessage(jid, buttonMessage, { quoted: q })
  ...
  conn.sendButtonImg = async(jid, buffer, content, footer, button1, row1, q, opts) => {
	  const buttons = [
	  {buttonId: row1, buttonText: {displayText: button1}, type: 1}
	  ]
const buttonMessage = {
...
return await conn.sendMessage(jid, buttonMessage, { quoted: q })
   ...
   conn.send2ButtonImg = async(jid, buffer, content, footer, button1, row1, button2, row2, q, opts) => {
	  const buttons = [
	   { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 }
	  ]
...
return await conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400,contextInfo: { externalAdReply :{
   ...
   conn.send3ButtonImg = async(jid, buffer, content, footer, button1, row1, button2, row2, button3, row3, q, opts) => {
	  const buttons = [
	  { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
          { buttonId: row3, buttonText: { displayText: button3 }, type: 1 }
